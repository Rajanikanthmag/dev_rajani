...
,offPoint_iata_code as dest
--,scheduled_departure_datetime as dep_time
,left(scheduled_departure_datetime, len(scheduled_departure_datetime) - 9) as dep_date
...
,leg_counter_availability_counter_gross
-- ,leg_counter_availability_counter_net
...
,rank() over (partition by processed_inventory_id order by processed_inventory_version desc) as rnk
 from dih_stream.dih_inv_pdl.inv_pdl_leg
 where scheduled_departure_datetime between current_timestamp() and date_add(current_timestamp(), 365)
--and boardPoint_iata_code = 'KUL'
--  where processed_inventory_id= 'MH-144-2023-11-29'
--  and leg_counter_cabin_code = 'J'
) where rnk = '1'
group by processed_inventory_id
,boardPoint_iata_code
,offPoint_iata_code
,scheduled_departure_datetime
,aircraft_equipment_aircraft_type
